...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Generate hourly forecast for next 24 hours with consistent time format
    const hourlyForecast = weatherData.hourly.temperature_2m.slice(0, 24).map((temp: number, index: number) => {
      const forecastTime = new Date(Date.now() + index * 3600000)
      return {
    ...
    // Fallback response - still return success: true with fallback data
    return NextResponse.json({
      success: true,
      location: {
        latitude: parseFloat(lat),
      ...
      forecast: {
        hourly: Array.from({length: 24}, (_, i) => ({
          ...
          cloudCover: 25 + Math.random() * 30,
          windSpeed: 12 + Math.random() * 8,
          auroraScore: 60 + Math.random() * 30
        })),
        bestViewing: {
